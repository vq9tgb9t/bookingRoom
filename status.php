...
if (!empty($roomIds)) {
    ...
    while ($b = $resultB->fetch_assoc()) {
        $rid = (int)$b['room_id'];
        if (!isset($bookingsByRoom[$rid])) $bookingsByRoom[$rid] = [];
...
<?php if ($info_message): ?>
  <div class="alert"><?= htmlspecialchars($info_message) ?></div>
<?php endif; ?>
...
<form method="get" class="filter-row">
    ...
    <input type="hidden" name="gedung" value="<?= htmlspecialchars($gedung) ?>">
    <input type="hidden" name="lantai" value="<?= (int)$lantai ?>">

    <div class="filter-group">
        <div class="filter-item">
            <span>Tanggal Cek Status</span>
            <input type="date" name="tanggal" value="<?= htmlspecialchars($tanggal_param) ?>">
        </div>
        <div class="filter-item">
            <span>Jam Cek Status</span>
            <input type="time" name="jam" value="<?= htmlspecialchars($jam_param) ?>">
        </div>
...
<!-- TAB GEDUNG -->
<div class="tab-row">
    <a href="?page=status&gedung=D&lantai=<?= $lantai ?>&tanggal=<?= $tanggal_param ?>&jam=<?= $jam_param ?>&mode=<?= $mode ?>&view_mode=<?= $view_mode ?><?= $apply_dt ? '&apply_dt=1' : '' ?>"
       class="tab <?= $gedung === 'D' ? 'active' : '' ?>">GEDUNG D</a>
    <a href="?page=status&gedung=S&lantai=<?= $lantai ?>&tanggal=<?= $tanggal_param ?>&jam=<?= $jam_param ?>&mode=<?= $mode ?>&view_mode=<?= $view_mode ?><?= $apply_dt ? '&apply_dt=1' : '' ?>"
...
<div class="floor-tabs">
    <?php for ($i = 1; $i <= 4; $i++): ?>
        <a href="?page=status&gedung=<?= $gedung ?>&lantai=<?= $i ?>&tanggal=<?= $tanggal_param ?>&jam=<?= $jam_param ?>&mode=<?= $mode ?>&view_mode=<?= $view_mode ?><?= $apply_dt ? '&apply_dt=1' : '' ?>"
...
?>
    <div class="room-grid-empty">
        Belum ada data ruangan untuk gedung <?= htmlspecialchars($gedung) ?> lantai <?= (int)$lantai ?>.
    </div>
<?php
else:
    foreach ($rooms as $room):
        ...
        // filter tampilan
        $skip = false;
        if ($mode === 'available' && $state !== 'free') $skip = true;
        if ($mode === 'occupied' && $state === 'free') $skip = true;

        if ($skip) continue;
?>
    <div class="room-card <?= $cardExtraClass ?>">
        <div class="room-title-row">
            <div class="room-name"><?= htmlspecialchars($room['kode_ruang']) ?></div>
            <div class="<?= $badgeClass ?>"><?= $badgeText ?></div>
        </div>
        <div class="room-desc">
            <?= htmlspecialchars($room['deskripsi'] ?: 'Ruang kelas besar, akses mudah, dekat lobby utama.') ?>
        </div>
        <div class="room-tags">
            <?php
            $fList = array_filter(array_map('trim', explode(',', (string)$room['fasilitas'])));
            if (empty($fList)) $fList = ['AC', 'Whiteboard', 'WiFi'];
            foreach ($fList as $f): ?>
                <span class="tag"><?= htmlspecialchars($f) ?></span>
            <?php endforeach; ?>
        </div>
        <?php if ($infoLine): ?>
            <div class="info-text"><?= htmlspecialchars($infoLine) ?></div>
        <?php endif; ?>
        <button 
            class="btn-primary btn-book"
            data-room-id="<?= $rid ?>"
            data-room-name="<?= htmlspecialchars($room['kode_ruang']) ?>"
        >Booking Ruangan</button>
    </div>
<?php
    endforeach;
endif;
